...
namespace MicrosoftResearch.Infer.Tutorials
{
	...
	public class BayesianPCAModel
	{
		...
		public BayesianPCAModel()
		{
			...
			vW[rD] = Variable.VectorGaussianFromMeanAndPrecision(
                            Vector.FromArray(0.0, 0.0),
                            PositiveDefiniteMatrix.Diagonal(vAlpha[rM])).ForEach(rD);
			// Latent variables are drawn from a standard Gaussian
			vZ = Variable.Array<Vector>(rN, rM).Named("Z");
			vZ[rN] = Variable.VectorGaussianFromMeanAndPrecision(0.0, 1.0).ForEach(rN);
	...
	public class BayesianPCA
	{
		...
		public void Run()
		{
			...
			// Set the dimensions
			bpca.vN.ObservedValue = numObs; //data.GetLength(0);
			bpca.vD.ObservedValue = numFeat; //data.GetLength(1);
			bpca.vM.ObservedValue = numComp; //6;
			// Set the priors
